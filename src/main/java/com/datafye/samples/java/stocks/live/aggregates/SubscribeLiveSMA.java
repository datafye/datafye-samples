...
public class SubscribeLiveSMA {
    ...
    private void onSMA(final String frequency, final String symbol, final long timestamp, final double price, final double macd) {
        System.out.println("<-- StocksSMA[" + frequency + "] {" + symbol + "," + timestamp + "," + price + "," + macd + "}");
        _numValuesReceived++;
    }

    @EventHandler
    final public void onStocksSecondSMA(final StocksSecondSMAMessage m) { onSMA("Second", m.getSymbol(), m.getTimestamp(), m.getPrice(), m.getMacd()); }

    @EventHandler
    final public void onStocksMinuteSMA(final StocksMinuteSMAMessage m) { onSMA("Minute", m.getSymbol(), m.getTimestamp(), m.getPrice(), m.getMacd()); }

    @EventHandler
    final public void onStocksHourSMA(final StocksHourSMAMessage m) { onSMA("Hour", m.getSymbol(), m.getTimestamp(), m.getPrice(), m.getMacd()); }
    ...
    public static void main(String args[]) throws Exception {
        ...
        if (!((Boolean)parser.getOptionValue(helpOption, false))) {
            ...
            // execute
            new SubscribeLiveSMA().run(dataset, frequency, symbols);
        }
        else {
            printUsage();
        }
