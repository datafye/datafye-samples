...
public class IsTickDownloadRunning {
    ...
    final private static void run(final String dataset) {
        ...
        IsStocksTickHistoryFetchRunningRequestMessage request = IsStocksTickHistoryFetchRunningRequestMessage.create();
        IsStocksTickHistoryFetchRunningResponseMessage response = client.isTickHistoryFetchRunning(request);
        boolean isRunning = response.getIsRunning();
        response.dispose();

        System.out.println("Tick history download is running: " + isRunning);
