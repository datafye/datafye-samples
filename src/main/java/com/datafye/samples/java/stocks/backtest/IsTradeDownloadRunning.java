...
public class IsTradeDownloadRunning {
    ...
    final private static void run(final String dataset) {
        ...
        IsStocksTradeHistoryFetchRunningRequestMessage request = IsStocksTradeHistoryFetchRunningRequestMessage.create();
        IsStocksTradeHistoryFetchRunningResponseMessage response = client.isTradeHistoryFetchRunning(request);
        boolean isRunning = response.getIsRunning();
        response.dispose();

        System.out.println("Trade history download is running: " + isRunning);
