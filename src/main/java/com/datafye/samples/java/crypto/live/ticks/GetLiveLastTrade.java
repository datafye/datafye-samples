...
public class GetLiveLastTrade {
    ...
    final private static void run(final String commaSeparatedSymbols) {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            GetLiveLastCryptoTradesRequestMessage request = GetLiveLastCryptoTradesRequestMessage.create();
            request.setSymbols(symbols);
            GetLiveLastCryptoTradesResponseMessage response = client.getLiveLastTrades(request);
