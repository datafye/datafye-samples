...
public class GetLiveEMA {
    ...
    final private static void run(final String symbol) {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            GetLiveCryptoEMAsRequestMessage request = GetLiveCryptoEMAsRequestMessage.create();
            request.setFrequency(OHLCFrequency.Minute);
            request.setSymbol(symbol);
            GetLiveCryptoEMAsResponseMessage response = client.getLiveEMAs(request);
            final int emasCount = response.getEmasCount();
            long stop = System.currentTimeMillis();

            // update total
            totalTime += (stop-start);
            totalCount += emasCount;
