...
public class GetLiveOHLC {
    ...
    final private static void run(final String symbol) {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            GetLiveCryptoOHLCsRequestMessage request = GetLiveCryptoOHLCsRequestMessage.create();
            request.setFrequency(OHLCFrequency.Minute);
            request.setSymbol(symbol);
            GetLiveCryptoOHLCsResponseMessage response = client.getLiveOHLCs(request);
