...
public class StartQuoteDownload {
    ...
    final private static void run(final Date date, final String[] symbols, final boolean wait) throws Exception {
        ...
        // start the download
        StartCryptoQuoteHistoryFetchRequestMessage request = StartCryptoQuoteHistoryFetchRequestMessage.create();
        request.serializer().startDate(date.getTime()).numDays(1).symbols(symbols).format(FileFormat.Binary).done();
        StartCryptoQuoteHistoryFetchResponseMessage response = client.startQuoteHistoryFetch(request);
        ...
        if (wait) {
            ...
            while (true) {
                ...
                IsCryptoQuoteHistoryFetchRunningRequestMessage statusRequest = IsCryptoQuoteHistoryFetchRunningRequestMessage.create();
                IsCryptoQuoteHistoryFetchRunningResponseMessage statusResponse = client.isQuoteHistoryFetchRunning(statusRequest);
