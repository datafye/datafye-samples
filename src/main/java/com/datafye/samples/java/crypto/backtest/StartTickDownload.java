...
public class StartTickDownload {
    ...
    final private static void run(final Date date, final String[] symbols, final boolean wait) throws Exception {
        ...
        // start the download
        StartCryptoTickHistoryFetchRequestMessage request = StartCryptoTickHistoryFetchRequestMessage.create();
        request.serializer().startDate(date.getTime()).numDays(1).symbols(symbols).format(FileFormat.Binary).done();
        StartCryptoTickHistoryFetchResponseMessage response = client.startTickHistoryFetch(request);
        ...
        if (wait) {
            ...
            while (true) {
                ...
                IsCryptoTickHistoryFetchRunningRequestMessage statusRequest = IsCryptoTickHistoryFetchRunningRequestMessage.create();
                IsCryptoTickHistoryFetchRunningResponseMessage statusResponse = client.isTickHistoryFetchRunning(statusRequest);
