...
public class StartTradeDownload {
    ...
    final private static void run(final Date date, final String[] symbols, final boolean wait) throws Exception {
        ...
        // start the download
        StartCryptoTradeHistoryFetchRequestMessage request = StartCryptoTradeHistoryFetchRequestMessage.create();
        request.serializer().startDate(date.getTime()).numDays(1).symbols(symbols).format(FileFormat.Binary).done();
        StartCryptoTradeHistoryFetchResponseMessage response = client.startTradeHistoryFetch(request);
        ...
        if (wait) {
            ...
            while (true) {
                ...
                IsCryptoTradeHistoryFetchRunningRequestMessage statusRequest = IsCryptoTradeHistoryFetchRunningRequestMessage.create();
                IsCryptoTradeHistoryFetchRunningResponseMessage statusResponse = client.isTradeHistoryFetchRunning(statusRequest);
