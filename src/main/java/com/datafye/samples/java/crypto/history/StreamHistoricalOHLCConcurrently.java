 ...
 */
package com.datafye.samples.java.crypto.history;
...
import com.datafye.samples.client.CryptoHistoryClient.HistoricalOHLCStream;

import com.datafye.samples.rest.domain.OHLC;

/**
 * Streams multiple historical OHLC streams concurrently using the Crypto History client.
 */
public class StreamHistoricalOHLCConcurrently {

    final private static class Streamer implements Runnable {
        final private class OHLCPopulator extends CryptoMinuteOHLCMessage.Deserializer.AbstractCallbackImpl {
            private OHLC _ohlc;

            void populate(OHLC ohlc, CryptoMinuteOHLCMessage message) {
                _ohlc = ohlc;
            ...
            @Override
            public void handleVolume(double val) {
                _ohlc.setVolume((long)val);
            }
        ...
        final private CryptoHistoryClient _client;
        ...
        Streamer(final CryptoHistoryClient client, final Date from, final int rate) {
            _client = client;
            _from = from;
            _rate = rate;
            _ohlc = new OHLC();
            _ohlcPopulator = new OHLCPopulator();
            _calendar = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
        }

        final private OpenHistoricalCryptoOHLCStreamResponseMessage openStream(final OpenHistoricalCryptoOHLCStreamRequestMessage request) {
         ...
         */
        @EventHandler
        final public void onStreamStart(final HistoricalCryptoOHLCStreamStartMessage message) {
         ...
         */
        @EventHandler
        final public void onStreamData(final CryptoMinuteOHLCMessage message) {
         ...
         */
        @EventHandler
        final public void onStreamEnd(final HistoricalCryptoOHLCStreamEndMessage message) {
        ...
        @EventHandler
        final public void onStreamError(final HistoricalCryptoOHLCStreamErrorMessage message) {
        ...
        final public void run() {
            try {
                ...
                OpenHistoricalCryptoOHLCStreamRequestMessage request = OpenHistoricalCryptoOHLCStreamRequestMessage.create();
                ...
                OpenHistoricalCryptoOHLCStreamResponseMessage response = openStream(request);
    ...
    public static void main(String args[]) throws Exception {
        ...
        if (!((Boolean)parser.getOptionValue(helpOption, false))) {
            ...
            // create the client
            final CryptoHistoryClient client = new CryptoHistoryClient("samples", String.valueOf(instance));
