...
public class GetLiveSMA {
    ...
    final private static void run(final String symbol) {
        // create the client
        CryptoAggClient client = new CryptoAggClient("samples", "0");
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            GetLiveCryptoSMAsRequestMessage request = GetLiveCryptoSMAsRequestMessage.create();
            request.setFrequency(OHLCFrequency.Minute);
            request.setSymbol(symbol);
            GetLiveCryptoSMAsResponseMessage response = client.getLiveSMAs(request);
            final int smasCount = response.getSmasCount();
            long stop = System.currentTimeMillis();

            // update total
            totalTime += (stop-start);
            totalCount += smasCount;
