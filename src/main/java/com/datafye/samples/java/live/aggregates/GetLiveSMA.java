 ...
 */
package com.datafye.samples.java.live.aggregates;
...
public class GetLiveSMA {
    ...
    final private static void run(final String dataset, final String symbol) {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            GetLiveStocksSMAsRequestMessage request = GetLiveStocksSMAsRequestMessage.create();
            request.setFrequency(OHLCFrequency.Minute);
            request.setSymbol(symbol);
            GetLiveStocksSMAsResponseMessage response = client.getLiveSMAs(request);
