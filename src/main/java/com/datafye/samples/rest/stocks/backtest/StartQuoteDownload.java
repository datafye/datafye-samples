 ...
 */
package com.datafye.samples.rest.stocks.backtest;
...
public class StartQuoteDownload {
    ...
    final private static void run(final String dataset, final String date, final String symbols, final boolean wait) throws Exception {
        ...
        // start the download
        HttpUrl url = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/backtest/history/quotes");
        ...
        if (wait) {
            ...
            while (true) {
                ...
                HttpUrl.Builder statusUrlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/backtest/history/quotes").newBuilder();
                statusUrlBuilder.addQueryParameter("dataset", dataset);
