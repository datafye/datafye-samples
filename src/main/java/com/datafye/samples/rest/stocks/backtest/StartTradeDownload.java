...
public class StartTradeDownload {
    ...
    final private static void run(final String dataset, final String date, final String symbols, final boolean wait) throws Exception {
        ...
        // start the download
        HttpUrl url = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/backtest/history/trades");
        ...
        if (wait) {
            ...
            while (true) {
                ...
                HttpUrl.Builder statusUrlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/backtest/history/trades").newBuilder();
