...
public class GetLiveEMA {
    ...
    final private static void run(final String dataset, final String symbol) throws Exception {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/live/agg/emas").newBuilder();
            ...
            GetLiveEMAResponse emaResponse = objectMapper.readValue(response.body().string(), GetLiveEMAResponse.class);
            long stop = System.currentTimeMillis();

            // update totals
            totalTime += (stop-start);
            totalCount += emaResponse.getEmas() != null ? emaResponse.getEmas().length : 0;
        }

        // average time
        System.out.println("Fetched '" + (totalCount/100) + "' EMA values for '" + symbol + "' in " + (totalTime/100) + " milliseconds.");
    }
