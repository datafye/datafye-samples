 ...
 */
package com.datafye.samples.rest.stocks.live.aggregates;
...
public class GetLiveSMA {
    static {
        System.setProperty("datafye-samples.api.endpoint", "api.rest.rumi.local:7776");
    }

    final private static void printUsage() {
        System.err.println("    [{-D, --dataset the stocks dataset to query (SIP or Synthetic, default=Synthetic)]");
        System.err.println("    [{-s, --symbol the symbol to fetch SMA values for (required)]");
        System.err.println("    [{-h, --help} print this help string]");
        System.exit(-1);
    }

    final private static void run(final String dataset, final String symbol) throws Exception {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/live/agg/smas").newBuilder();
            urlBuilder.addQueryParameter("dataset", dataset);
            ...
            GetLiveSMAResponse smaResponse = objectMapper.readValue(response.body().string(), GetLiveSMAResponse.class);
            long stop = System.currentTimeMillis();

            // update totals
            totalTime += (stop-start);
            totalCount += smaResponse.getSmas() != null ? smaResponse.getSmas().length : 0;
        }

        // average time
        System.out.println("Fetched '" + (totalCount/100) + "' SMA values for '" + symbol + "' in " + (totalTime/100) + " milliseconds.");
    }
