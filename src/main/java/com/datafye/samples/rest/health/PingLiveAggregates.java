...
public class PingLiveAggregates {
    static {
    ...
    final private static void run() throws Exception {
        ...
        HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/live/agg/ping").newBuilder();
        ...
        if (response.isSuccessful()) {
            System.out.println("Live aggregates subsystem [Synthetic] is healthy.");
        }
        else {
            System.err.println("Live aggregates subsystem [Synthetic] is unreachable. (HTTP " + response.code() + ")");
            System.exit(1);
        }
    }
