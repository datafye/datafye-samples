...
public class PingLiveFeed {
    ...
    final private static void run() throws Exception {
        ...
        HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/stocks/live/ping").newBuilder();
        ...
        if (response.isSuccessful()) {
            System.out.println("Live feed subsystem [Synthetic] is healthy.");
        }
        else {
            System.err.println("Live feed subsystem [Synthetic] is unreachable. (HTTP " + response.code() + ")");
            System.exit(1);
        }
    }
