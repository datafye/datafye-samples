...
public class GetHistoricalOHLC {
    ...
    final private static void run(final String symbol, final String frequency, final Date from, final Date to) throws Exception {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/history/ohlcs").newBuilder();
            ...
            GetHistoricalCryptoOHLCResponse ohlcResponse = objectMapper.readValue(response.body().string(), GetHistoricalCryptoOHLCResponse.class);
