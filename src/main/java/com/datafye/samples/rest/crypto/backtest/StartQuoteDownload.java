...
public class StartQuoteDownload {
    ...
    final private static void run(final String date, final String symbols, final boolean wait) throws Exception {
        ...
        // start the download
        HttpUrl url = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/backtest/history/quotes");
        ...
        if (wait) {
            ...
            while (true) {
                ...
                HttpUrl.Builder statusUrlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/backtest/history/quotes").newBuilder();
