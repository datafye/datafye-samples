...
public class GetLiveLastTrade {
    ...
    final private static void run(final String symbols) throws Exception {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/live/trades/latest").newBuilder();
            ...
            GetLiveCryptoLastTradeResponse tradeResponse = objectMapper.readValue(response.body().string(), GetLiveCryptoLastTradeResponse.class);
