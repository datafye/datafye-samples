 ...
 */
package com.datafye.samples.rest.crypto.live.ticks;
...
public class GetLiveTopOfBook {
    ...
    final private static void run(final String symbols) throws Exception {
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            HttpUrl.Builder urlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/live/quotes/top-of-book").newBuilder();
            ...
            GetLiveCryptoTopOfBookQuotesResponse quotesResponse = objectMapper.readValue(response.body().string(), GetLiveCryptoTopOfBookQuotesResponse.class);
