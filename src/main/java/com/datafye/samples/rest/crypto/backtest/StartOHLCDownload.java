...
public class StartOHLCDownload {
    ...
    final private static void run(final String date, final String symbols, final String frequency, final boolean wait) throws Exception {
        ...
        // start the download
        HttpUrl url = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/backtest/history/ohlcs");
        ...
        if (wait) {
            ...
            while (true) {
                ...
                HttpUrl.Builder statusUrlBuilder = HttpUrl.parse("http://" + Config.getValue("datafye-samples.api.endpoint") + "/datafye-api/v1/crypto/backtest/history/ohlcs").newBuilder();
