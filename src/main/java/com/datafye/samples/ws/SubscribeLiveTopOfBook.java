...
/**
 * Streams live top-of-book quotes over a WebSocket from a Datafye Deployment.
 *
 * Connects to {@code /datafye-ws/v1/stocks/live/quotes}, subscribes to the given symbols, and prints each
 * live quote frame as it arrives. Available for the SIP, Synthetic and Crypto datasets. For stocks, a halt
 * or resume of a subscribed symbol is delivered as a {@code halt}/{@code resume} control frame.
 *
 * Subscribe request: {@code {"action":"subscribe","dataset":"SIP","symbols":["AAPL"]}}
 */
public class SubscribeLiveTopOfBook {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        if ((Boolean) parser.getOptionValue(helpOption, false) || dataset == null || symbols == null) {
            printUsage();
        }

        final String subscribe = "{\"action\":\"subscribe\",\"dataset\":\"" + dataset
                + "\",\"symbols\":" + StreamClient.symbolsArray(symbols) + "}";
        StreamClient.stream(StreamClient.wsUrl("stocks/live/quotes"), subscribe, seconds);
    }
}
